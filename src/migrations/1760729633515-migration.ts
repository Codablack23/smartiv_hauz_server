...
export class Migration1760729633515 implements MigrationInterface {
    name = 'Migration1760729633515'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`resources\` (\`id\` varchar(36) NOT NULL, \`title\` longtext NOT NULL, \`banner_url\` longtext NOT NULL, \`file_url\` longtext NOT NULL, \`slug\` longtext NOT NULL, \`description\` longtext NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`team_members\` ADD \`email\` longtext NULL`);
        await queryRunner.query(`ALTER TABLE \`team_members\` ADD UNIQUE INDEX \`IDX_a88b84bdd6391e3d877a59fbb7\` (\`email\`)`);
        await queryRunner.query(`ALTER TABLE \`clients\` ADD \`company_email\` longtext NULL`);
        await queryRunner.query(`ALTER TABLE \`clients\` ADD UNIQUE INDEX \`IDX_def072ac9736e6eefbb59315d8\` (\`company_email\`)`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`clients\` DROP INDEX \`IDX_def072ac9736e6eefbb59315d8\``);
        await queryRunner.query(`ALTER TABLE \`clients\` DROP COLUMN \`company_email\``);
        await queryRunner.query(`ALTER TABLE \`team_members\` DROP INDEX \`IDX_a88b84bdd6391e3d877a59fbb7\``);
